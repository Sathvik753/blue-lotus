...
\begin{abstract}
\noindent
We present the Blue Lotus Stress-Testing Engine, a system that maps a single
return series to a forward-looking distribution of drawdown, tail-loss, and
recovery outcomes. The engine integrates a deterministic three-state volatility
regime model, Bayesian shrinkage of the regime parameters, Generalized Pareto
tail estimation via Peaks-over-Threshold Extreme Value Theory, and
regime-switching Monte Carlo path generation with a drawdown-conditional blend and
a hard loss floor. An optional Esscher importance resampler and bootstrapped
confidence intervals on every reported metric complete the pipeline. A Model
Fragility Index, defined as the mean Wasserstein-1 distance between the baseline
and parameter-perturbed drawdown distributions, provides a single auditable measure
of the estimates' sensitivity to parameter uncertainty.

The method is evaluated under a strict out-of-sample protocol. For nine named
market crises and ten liquid asset proxies, the engine is re-estimated using only
the history available prior to each crisis and its predicted drawdown distribution
is compared with the realized outcome, yielding 89 crisis cells; an identical
protocol applied to 213 calm windows constitutes the calibration control. On calm
data the $5\%$ and $1\%$ tail bounds are breached in $2.8\%$ and $0.9\%$ of cells
respectively, neither rejectable by a Kupiec test ($p=0.11$, $p=0.93$). Under the
nine crises these rates rise to $56\%$ and $31\%$, degrading monotonically with
severity and failing entirely only at the two fastest systemic shocks on record.
The engine thus reproduces ordinary risk faithfully and reports, rather than
conceals, the residual tail gap that history alone cannot close.
\end{abstract}
...
Given a history of daily returns --- a market index, a fund, or the profit-and-loss
of a trading strategy --- the engine addresses a single question: conditional on
the return-generating behaviour implied by the series, what is the distribution of
adverse outcomes over a forward horizon? The engine does not forecast a price level
or a direction. It simulates a large ensemble of forward paths and reports the
\emph{distribution} of three quantities of primary interest to an allocator: the
expected maximum drawdown, the severity of losses in the extreme lower percentiles,
and the time required to recover. Each reported statistic is accompanied by a
confidence interval, and every run is exportable as a structured JSON object or a
formatted PDF.

\subsection{Limitations of stationary Monte Carlo}
The standard approach draws returns from a single fixed distribution and simulates
forward. It is analytically convenient but deficient in two respects that are
material to risk measurement.

First, returns are not generated by a single stationary law. Volatility clusters,
losses exhibit serial dependence, and the probability of a large decline
conditional on a recent sequence of declines substantially exceeds its
unconditional value. A model that ignores this structure understates sustained
crisis drawdowns while overstating isolated single-day moves.

Second, tail observations are scarce. A fifteen-year equity history contains
approximately four thousand daily observations, of which only a few hundred
correspond to genuine stress. Any tail model estimated from those few hundred
points carries substantial estimation error, and unadjusted simulation propagates
that error without any accompanying measure of its reliability.

\subsection{Contributions}
The Blue Lotus engine addresses both deficiencies and incorporates an explicit
model-risk diagnostic. This paper makes four contributions:
\begin{enumerate}[leftmargin=1.6em, itemsep=2pt]
  ...
  \item A transparent assessment (Sections~\ref{sec:discussion}--\ref{sec:limits})
  of the conditions under which the engine is well calibrated and those under
  which, by construction, no history-calibrated model can be accurate.
\end{enumerate}
...
The engine is a pipeline of seven stages. Each stage is a self-contained module
with typed inputs and outputs, so that any stage may be inspected, replaced, or
...
All stages downstream of stage~1 are deterministic conditional on a fixed random
seed, so any run is exactly reproducible --- a property on which the backtesting
protocol depends.
...
Each modelling choice trades statistical sophistication against auditability. The
engine consistently favours methods whose outputs admit a direct interpretation by
a risk officer.

\paragraph{Deterministic regimes over a hidden Markov model.}
An earlier version of the engine employed a three-state Gaussian HMM estimated by
Baum--Welch. Although theoretically attractive, it produced crisis fractions that
varied substantially across assets and required manual reseeding to avoid
degenerate states --- properties ill-suited to a production risk system. The
current detector assigns regimes by explicit rules on rolling volatility and trend
(Section~\ref{sec:regime}). Each label admits a one-sentence justification, the
crisis fraction is calibrated by construction through its percentile thresholds,
and the model contains no latent state subject to misconvergence.
...
When the simulated mean or Expected Shortfall departs from the EVT and regime
targets, an Esscher exponential tilt reweights the paths to restore consistency.
The tilt is optional and self-disabling: if its solver fails to converge, the
engine reverts to identity weights and preserves the historical measure, rather
than imposing an unstable correction to satisfy a target.

\paragraph{Bootstrap intervals throughout.}
A risk estimate reported without an interval conveys false precision. The engine
resamples paths to attach a 90\% confidence band to every reported scalar, so that
estimation uncertainty is quantified explicitly rather than left implicit.
...
Let $\{r_t\}_{t=1}^T$ be a daily return series. Prior to estimation, the series is
validated as \emph{decimal} returns rather than percentages. The admissible
per-day range is set intentionally wide --- up to an order-of-magnitude single-day
move --- so that legitimately high-volatility inputs, such as digital assets or
individual securities subject to gap risk, are not rejected as malformed. Returns
are then winsorised at the 1st and 99th percentiles, clipping extreme observations
to those boundaries so that a small number of points cannot dominate the moment
and tail estimates. Raw mean, standard deviation, skewness, and excess kurtosis
are recorded \emph{prior} to winsorising for reporting. Annualised volatility is
...
The engine is deployed as a production service rather than a research script
alone. The numerical core (\texttt{engine/core.py}) is implemented in NumPy/SciPy
and has no web dependencies. A FastAPI application (\texttt{api/}) wraps the core,
submitting long-running computations as background jobs and persisting runs and
results through an asynchronous SQLAlchemy layer (\texttt{db/}). The authentication
subsystem --- JWT tokens and API keys --- is implemented in the codebase but
disabled in the current build, which executes every request as a single shared
workspace user; re-enabling per-account authentication requires a single
dependency substitution. Results are serialised to a JSON-safe payload by a
dedicated module (\texttt{engine/serializer.py}) and to a formatted tear-sheet by
a ReportLab module (\texttt{reports/pdf.py}); both are exposed as direct exports in
the interface. Two front-ends consume the API: a React single-page application
(\texttt{web/}) for interactive use and a Streamlit application (\texttt{frontend/})
for expedited analysis. The identical codebase runs locally under Docker Compose
and in production on Google Cloud Run, and every run is reproducible from its seed.
...
The mean one-year maximum drawdown is $-14.1\%$ and the 5th-percentile drawdown is
$-37.0\%$, a reasonable tail benchmark for position sizing on a $17\%$-volatility
index. The elevated never-recover rate requires qualification: it is a horizon
artefact rather than a statement about the long-run index. Paths that enter
drawdown late in the $252$-day window have insufficient remaining horizon to
recover and are recorded as unrecovered, although a longer horizon would in most
cases capture the subsequent recovery.
...
The single-asset section establishes that the engine produces internally coherent
estimates. Internal coherence, however, is a necessary but not a sufficient
condition. The material question is whether the predicted distributions in fact
contained the realized outcomes. This question is addressed strictly
out-of-sample.
...
crisis set the statistic quantifies the conditional exceedance rate rather than
functioning as an acceptance test.
...
Table~\ref{tab:headline} and Figure~\ref{fig:calib} establish the control. On calm
windows the engine breaches its $5\%$ bound in $2.8\%$ of cells and its $1\%$ bound
in $0.9\%$, both statistically indistinguishable from nominal (Kupiec $p=0.11$ and
$0.93$). The mean PIT rank of $0.69$ indicates that the engine is mildly
conservative on ordinary data, predicting marginally more risk than is realized.
For a risk system this is the prudent direction of error, and it is a precondition
for interpreting the crisis results: a model that generated excessive exceptions
under calm conditions would provide no basis for assessing its behaviour under
stress.
...
The crisis results (Table~\ref{tab:events}, Figures~\ref{fig:scatter}--\ref{fig:eventrank})
invert the control. Coverage degrades monotonically with severity: the 2015--16
China/oil selloff is fully covered and the 2023 regional-bank episode substantially
so, whereas every COVID-2020 cell and nearly every 2008 cell breaches even the
$1\%$ bound. Realized crisis drawdowns exhibit a mean PIT rank of $0.14$, deep
within the predicted lower tail, and the two most severe systemic shocks fall
essentially at rank zero, the most extreme exceedance the statistic can record.
...
Four cells illustrate the range of outcomes. \textbf{SPY in COVID-2020}: realized
$-38.2\%$ against a predicted $p5$ of $-8.6\%$ ($p1$ $-11.9\%$), rank $0.000$; the
most rapid $30\%$ equity drawdown on record lies outside the support implied by
pre-2020 data. \textbf{XLF in 2008}: realized $-103.6\%$ (additive) against $p5$
$-38.2\%$, rank $0.000$, reflecting a solvency crisis internal to the financial
sector. \textbf{SPY in 2015--16}: realized $-13.3\%$ against $p5$ $-20.3\%$, rank
$0.25$, a correction fully contained within the predicted distribution.
\textbf{SPY in 2023}: realized $-3.4\%$ against $p5$ $-6.7\%$, rank $0.28$, again
contained and correctly bounded by the engine.

% =============================================================== 8 Discussion
\section{Discussion}
\label{sec:discussion}

The two regimes support a single conclusion. The calm control demonstrates that
the engine is correctly specified for ordinary conditions: at both the $5\%$ and
the $1\%$ tail level its out-of-sample exception rates are statistically
indistinguishable from nominal, with error biased toward conservatism. The crisis
study demonstrates that the outcomes which do breach are precisely those that a
history-calibrated model cannot anticipate --- the most abrupt and reflexive
systemic shocks, in which volatility, correlation, and liquidity dislocate more
rapidly than any trailing estimation window can represent. Prior to March 2020,
the March 2020 event was absent from the data.

This is not a defect specific to Blue Lotus; it is a property of the estimation
problem. Any model whose tail is calibrated to realized returns is bounded by its
worst observed history. The contribution of the backtest is to \emph{quantify} the
residual gap rather than to obscure it, and to establish that the gap is monotone
in severity and concentrated in the exposures that carry systemic tail risk.
Accordingly, the production engine does not rely on the empirical tail alone: its
crisis-regime structure, drawdown-conditional blend, and operator-configurable loss
floor are designed to extend the simulated distribution into the region that the
backtest identifies as beyond historical reach. The appropriate deployment is
therefore to apply the engine as calibrated here for ordinary risk measurement and
to engage the crisis overlay when sizing for systemic scenarios.
...
The Blue Lotus engine maps a return series to a fully characterised, bootstrap-
bounded distribution of drawdown, tail-loss, and recovery outcomes, constructed
from auditable components and delivered as a reproducible production service. Under
a strict out-of-sample protocol it is statistically well calibrated under ordinary
market conditions --- not rejectable at either the $5\%$ or the $1\%$ tail level
across $213$ calm cells --- and its behaviour under the nine largest crises of the
past two decades is fully characterised, with coverage that degrades monotonically
in severity and a discrete failure only at the two most severe systemic shocks.
This is the appropriate standard for such a system: not that it predict
unprecedented events, but that it delineate precisely the boundary of its validity.

Three extensions follow directly. The regime detector could incorporate exogenous
stress signals --- implied volatility, credit spreads, and funding indicators ---
to improve crisis identification in advance of realized volatility. The
multi-asset dependence model warrants the out-of-sample treatment applied here to
the univariate case, validated against joint drawdown events. Finally, the Model
Fragility Index could itself be backtested, by testing whether high-fragility
periods precede realized instability. In each case the evaluative standard is the
one adopted throughout: quantify the residual gap, and report it.
